...
interface BudgetData {
  budget: { id: number; budget_amount: number } | null;
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setData(d);
        setBudgetInput(d.budget ? String(d.budget.budget_amount) : "");
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleJudge = () => {
    ...
    if (afterPurchase > budget) {
      setJudgment({
        ok: false,
        message: "❌ 구매하지 마세요",
        detail: `이 금액을 사용하면 이번 달 예산(${formatKRW(budget)})을 ${formatKRW(afterPurchase - budget)} 초과해요. 남은 예산은 ${formatKRW(remaining)}이에요.`,
      });
    } else if (percentAfter > 90) {
      setJudgment({
        ok: false,
        message: "⚠️ 신중하게 고려하세요",
        detail: `구매 후 예산의 ${percentAfter.toFixed(0)}%를 사용하게 되어요. 앞으로 ${daysLeft}일이 남아있어서 빠듯할 수 있어요.`,
      });
      ...
      setJudgment({
        ok: true,
        message: "✅ 구매해도 괜찮아요",
        detail: `구매 후 예산의 ${percentAfter.toFixed(0)}%만 사용돼요. 남은 ${daysLeft}일 동안 ${formatKRW(Math.round((budget - afterPurchase) / Math.max(daysLeft, 1)))}씩 쓸 수 있어요.`,
      });
    }
  };

  const budget = data?.budget?.budget_amount ?? 0;
  const spent = data?.monthlyExpense ?? 0;
  const remaining = budget - spent;
  const usedPct = budget > 0 ? Math.min((spent / budget) * 100, 100) : 0;
  const daysInMonth = new Date(year, month, 0).getDate();
  const today = now.getDate();
  const idealPct = (today / daysInMonth) * 100;
        ...
        <div className="px-4 mt-4 space-y-4">
          ...
          <div className="bg-white rounded-2xl shadow-sm px-5 py-5">
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-xs text-toss-text-ter">{monthLabel}</p>
                <p className="text-base font-bold text-toss-text mt-0.5">이번 달 예산</p>
              </div>
              <button
            ...
            {budget > 0 ? (
              <>
                <div className="flex justify-between mb-2">
                  <p className="text-2xl font-bold text-toss-text">{formatKRW(budget)}</p>
                ...
                <div className="relative h-3 bg-toss-surface rounded-full overflow-hidden mb-2">
                  {/* 이상적인 진도 (점선) */}
                  <div
                    className="absolute top-0 bottom-0 w-0.5 bg-toss-text-ter z-10"
                    style={{ left: `${idealPct}%` }}
                  />
                  {/* 실제 사용 */}
                  <div
                ...
                <div className="flex justify-between">
                  <p className="text-xs text-toss-text-ter">
                    남은 금액 <span className={clsx("font-semibold", remaining < 0 ? "text-toss-red" : "text-toss-blue")}>
                      {formatKRW(remaining)}
                    </span>
                  </p>
                ...
                <p className="text-xs text-toss-text-ter mt-1">
                  {today}일 기준 이상적 지출: {formatKRW(Math.round((budget / daysInMonth) * today))}
                  {usedPct > idealPct + 5
                    ? " · 지출이 빠른 편이에요"
                    : usedPct < idealPct - 10
                    ? " · 절약하고 있어요 👍"
              ...
              <div className="text-center py-4">
                <p className="text-sm text-toss-text-ter">이번 달 예산을 설정해보세요</p>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm px-5 py-5">
            ...
            <p className="text-xs text-toss-text-ter mb-4">구매하려는 금액을 입력하면 판단해드려요</p>

            {budget <= 0 ? (
              <p className="text-sm text-toss-text-ter text-center py-4">먼저 이번 달 예산을 설정해주세요</p>
            ) : (
              <>
                ...
                {judgment && (
                  <div className={clsx("rounded-xl px-4 py-4 mt-2", {
                    "bg-green-50": judgment.ok,
                    "bg-red-50": !judgment.ok,
                  })}>
                    <p className={clsx("text-sm font-bold mb-1.5", {
                      "text-green-700": judgment.ok,
                      "text-toss-red": !judgment.ok,
                    })}>
                      {judgment.message}
                    </p>
          ...
          <div className="bg-white rounded-2xl shadow-sm px-5 py-4">
            <p className="text-sm font-bold text-toss-text mb-3">오늘 지출</p>
            <div className="flex items-center justify-between">
              <p className="text-2xl font-bold text-toss-red">{formatKRW(data?.todayExpense ?? 0)}</p>
              {budget > 0 && (
                <p className="text-xs text-toss-text-ter">
                  하루 권장 {formatKRW(Math.round(budget / daysInMonth))}
                </p>
              )}
            </div>
          </div>
